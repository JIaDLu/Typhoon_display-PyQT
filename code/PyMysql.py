...
class MSSQL(object):
    ...
    def select_typhoon_year(self, NAME, table_name):
        sql = "select distinct {} from {}".format(NAME, table_name)
        count = self._cur.execute(sql)
        ret = self._cur.fetchall()
        return ret


    def select_typhoon_name(self, NAME, table_name, YEAR, NUM):
        sql = "select distinct {} from {} where {} = '{}'".format(NAME, table_name, YEAR, NUM)
        count = self._cur.execute(sql)
        ret = self._cur.fetchall()
        return ret

    def select_typhoon_track(self,LAT,LON, table_name, YEAR, NUM, NAME, NAME_in):
        sql = "select {},{} from {} where {} = '{}' and {} = '{}'".format(LAT, LON, table_name, YEAR, NUM, NAME, NAME_in)
        count = self._cur.execute(sql)
        ret = self._cur.fetchall()
        return ret

    def select_typhoon_detail(self, table_name, YEAR, NUM, NAME, NAME_in):
        sql = "select * from {} where {} = '{}' and {} = '{}'".format(table_name, YEAR, NUM, NAME, NAME_in)
        self._cur.execute(sql)
        ret = self._cur.fetchall()
        return ret
